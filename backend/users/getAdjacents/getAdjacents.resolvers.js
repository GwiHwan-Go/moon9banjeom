   ...
   Query : {
       getAdjacents : protectedResolver(async(_,__,{loggedInUser}) => {
           ...
           let seenList = await client.seen.findMany({
            ...
            let photoList = [];
            let index = 0;
            while (index<result.length && photoList.length<8){
                ...
                photoList = [...new Set(
                    [...photoList, ...userList?.map(obj=>obj.photoId)])]
                index++;
            }//photoList = final recommendation according to adjacent user's liked list.
